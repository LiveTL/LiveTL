...
test('injects LiveTL and mounts a clean HyperChat embed', async ({ page }) => {
  ...
  await expect(chat.locator('#ltl-wrapper > button')).toHaveText(['Open LiveTL', 'TL Popout', 'Embed TLs', '']);
  ...
  await expect(
    hyperchat.locator(
      'link[name="www-player"], link[href*="www-player.css"], link[name="embed-ui"], link[href*="ytembeds"]',
    ),
  ).toHaveCount(0);
  await expect(
    hyperchat.locator('#player, #player-controls, .player-unavailable, yt-live-chat-app, ytd-app, ytm-app'),
  ).toHaveCount(0);
