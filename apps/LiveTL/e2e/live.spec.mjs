...
test.describe('@live real YouTube', () => {
  ...
  const openChat = async (page, url) => {
    ...
    try {
      ...
      throw new Error(
        `YouTube did not expose chat (CAPTCHA, ads, region, or upstream page change): ${text.slice(0, 300)}`,
        { cause: error },
      );
    }
  ...
  const openEmbed = async (page, url, waitForTldex = false) => {
    ...
    await expect.poll(() => video.evaluate((element) => element.currentTime)).toBeGreaterThan(initialTime + 0.5);
    return embed;
  };

  const seek = async (page, seconds) => {
    const player = page.locator('#movie_player');
    await expect.poll(() => player.evaluate((element) => typeof element.seekTo)).toBe('function');
  ...
  test('shows the exact YouTube-chat VOD translation', async ({ page }) => {
    ...
    const message = embed
      .locator('.message-display > .message')
      .filter({ hasText: 'just being beside peko makes me happy' })
      .first();
