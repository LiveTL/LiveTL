          ...
          const refreshHeights = (originalEvent: MouseEvent | undefined = undefined): void => {
            ...
            if (originalEvent) {
              const moveListener = (event: MouseEvent): void => {
                const chatRoomHeight = Math.min(
                  clientRect.height + (event.clientY - originalEvent.clientY),
                  maxChatRoomHeight,
                );
...
let first = true;
/** Recursive setTimeout to keep injecting whenever chat unloads/reloads when navigating thru the site. */
