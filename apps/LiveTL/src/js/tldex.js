...
const getVideoDataWithRetry = async (videoLink) => {
  ...
  if (dt?.start_actual || dt?.available_at)
    return dt.start_actual ? Date.parse(dt.available_at) : Date.parse(dt.start_actual);
};
...
const getScript = async (url, meta) =>
  await dexfetch(url)
    .then((e) =>
      e
        .filter((c) => !c.is_owner)
...
export const getArchive = (videoLink) =>
  readable(null, async (set) => {
    ...
    const unsubscribes = scripts.map(archiveStreamFromScript).map((stream) =>
      stream.subscribe((tl) => {
        if (tl && enableTldexTLs.get() && !mchadUsers.get(tl.author)) {
          set(tl);
        }
      }),
    );
...
export const getRoomTranslations = (videoLink, langCode) =>
  derived(streamRoom(videoLink, langCode), (data, set) => {
...
export const getLiveTranslations = (videoLink) =>
  readable(null, async (set) => {
    await languages.loaded;
    const unsubscribes = languages
      .get()
      .map((language) => languageNameCode[language])
      .map((e) => e.code)
      .map((langcode) =>
        getRoomTranslations(videoLink, langcode).subscribe((msg) => {
