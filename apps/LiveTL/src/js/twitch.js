 ...
 *
 * It more or less does operationName(variables) on the server and returns response. Hash has to be of the exact query,
 * so you must get that from the network devtools. It will stay the same regardless of variables.
 *
 ...
 */
const gqlFetch = (operationName, variables, hash) =>
  fetch(twitchGQL, {
    method: 'POST',
    headers: {
...
export const getStreamer = (videoID) =>
  gqlFetch('ChannelVideoCore', { videoID }, 'cf1ccf6f5b94c94d662efec5223dfb260c9f8bf053239a76125a58118769e8e2')
    .then((data) => data.video.owner.login)
    .catch(() => null);
...
export const getStartTime = async (videoID, channelLogin) => {
  ...
  return await gqlFetch(
    ...
    .then((data) => new Date(data.video.createdAt).getTime())
    .catch(() => null);
};
