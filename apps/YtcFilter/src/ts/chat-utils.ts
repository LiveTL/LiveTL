...
export const getFrameInfoAsync = async (): Promise<Chat.UncheckedFrameInfo> => {
  return await new Promise((resolve) => chrome.runtime.sendMessage({ type: 'getFrameInfo' }, resolve));
};
...
const youtubePlayerShellSelectors = [
  '#player',
  '#player-controls',
  '.player-unavailable',
  'yt-live-chat-app',
...
export const frameIsReplay = (): boolean => {
  try {
    return window.location.href.startsWith(`${location.protocol + '//' + location.host}/live_chat_replay`);
  } catch (e) {
    return false;
  }
};
 ...
 * Type predicates
 */
export const isPaidMessageRenderer = (r: Ytc.Renderers): r is Ytc.PaidMessageRenderer =>
  'purchaseAmountText' in r && 'bodyBackgroundColor' in r;

export const isPaidStickerRenderer = (r: Ytc.Renderers): r is Ytc.PaidStickerRenderer =>
  'purchaseAmountText' in r && 'moneyChipBackgroundColor' in r;

export const isMembershipRenderer = (r: Ytc.Renderers): r is Ytc.MembershipRenderer => 'headerSubtext' in r;
...
export const responseIsAction = (r: Chat.BackgroundResponse): r is Chat.Actions => actionTypes.has(r.type);
...
export const isPrivileged = (types: string[]): boolean => types.some(privilegedTypes.has.bind(privilegedTypes));
...
export const textIsObsoleteMemberEmoji = (text: string): boolean => {
  ...
  return nonWhitespace.length > 0 && [...nonWhitespace].every((char) => char === OBSOLETE_MEMBER_EMOJI_PLACEHOLDER);
};

export const isAllEmoji = (a: Chat.MessageAction): boolean =>
  a.message.message.length !== 0 &&
  a.message.message.every(
    (m) => m.type === 'emoji' || (m.type === 'text' && (m.text.trim() === '' || textIsObsoleteMemberEmoji(m.text))),
  );
...
type ReconnectingPort<T extends Chat.Port> = Partial<
  Pick<T, 'name' | 'disconnect' | 'postMessage' | 'onMessage' | 'onDisconnect'>
> & { destroy: () => void };
...
export const useReconnect = <T extends Chat.Port>(connect: () => Promise<T>): ReconnectingPort<T> => {
  ...
  return {
    ...
    disconnect(...args) {
      return actualPort?.disconnect(...args);
    },
    postMessage(...args) {
      return actualPort?.postMessage(...args);
    },
    get onMessage() {
      return actualPort?.onMessage;
    },
    get onDisconnect() {
      return actualPort?.onDisconnect;
    },
    destroy: () => {
      // eslint-disable-next-line @typescript-eslint/no-misused-promises
